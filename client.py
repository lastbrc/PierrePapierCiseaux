...
# c = conn.cursor()
# # req = 'CREATE TABLE joueurs(id integer primary key autoincrement, pseudo text)'
# # c.execute(req)
# conn.commit()
...
lbl_name = tk.Label(top_welcome_frame, text="Pseudo:")
lbl_name.pack(side=tk.LEFT)
ent_name = tk.Entry(top_welcome_frame)
ent_name.pack(side=tk.LEFT)
btn_connect = tk.Button(
    top_welcome_frame, text="Connecté", command=lambda: connect(),)
...
lbl_your_name = tk.Label(
    top_left_frame, text="Ton pseudo: " + your_name, font="Helvetica 13 bold"
)
...
lbl_line = tk.Label(
    middle_frame, text="**** Connecté ****", font="Helvetica 13 bold", foreground="blue"
).pack()
lbl_line = tk.Label(
    middle_frame, text="***********************************************************"
).pack()
...
lbl_your_choice = tk.Label(
    round_frame, text="Ton choix: " + "Null", font="Helvetica 13 bold"
)
lbl_your_choice.pack()
lbl_opponent_choice = tk.Label(
    round_frame, text="Choix de l'adversaire: " + "Null")
...
def connect():
    ...
    if len(ent_name.get()) < 1:
        tk.messagebox.showerror(
            title="Erreur!!!", message="Vous DEVEZ saisir votre prénom <e.g. John>"
        )
    else:
        your_name = ent_name.get()
        lbl_your_name["text"] = "Votre pseudo : " + your_name
        connect_to_server(your_name)
        # c.execute("INSERT INTO joueurs(pseudo) VALUES('?')")
        # conn.commit()
        # conn.close()
...
def count_down(my_timer, nothing):
    ...
    lbl_game_round["text"] = "Round " + str(game_round) + " commence dans"

    while my_timer > 0:
        my_timer = my_timer - 1
        print("la minuterie du jeu est: " + str(my_timer))
...
def choice(arg):
    ...
    your_choice = arg
    lbl_your_choice["text"] = "Ton choix: " + your_choice

    if client:
        dataToSend = "Tour" + str(game_round) + your_choice
    ...
    while True:
        ...
        if from_server.startswith("welcome"):
            if from_server == "welcome1":
                lbl_welcome["text"] = (
                    "Bienvenue " + your_name + "! En attente du joueur 2"
                )
            elif from_server == "welcome2":
                lbl_welcome["text"] = (
                    "Bienvenue " + your_name + "! Le jeu va bientot commencer"
                )
            lbl_line_server.pack()

        elif from_server.startswith("opponent_name$"):
            opponent_name = from_server.replace("opponent_name$", "")
            lbl_opponent_name["text"] = "Opposant: " + opponent_name
            top_frame.pack()
            middle_frame.pack()
            ...
            if game_round == TOTAL_NO_OF_ROUNDS:
                ...
                if your_score > opponent_score:
                    final_result = "(Tu a gagné!!!)"
                    color = "green"
                elif your_score < opponent_score:
                    final_result = "(Tu a perdu!!!)"
                    color = "red"
                else:
                    final_result = "(Draw!!!)"
                    color = "black"
